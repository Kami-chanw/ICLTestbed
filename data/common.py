...
def split_generators(
    ...
    Args:
        expand_path_fn (function): a method requires `file_type` and `split_name` as input and returns a `pathlib.Path` object pointing to the full path of `file_type` (e.g. questions, annotations and images).
        type_split_file_dict (Dict[str, Dict[str, str]]): a multi dict that contains file type -> split name -> file name.
        verbose (bool, defaults to be True): whether to warn if a split files are not exist.
    """
    splits = set(
        [
            file_name
